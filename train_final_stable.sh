...
# FINAL STABLE SCRIPT v2
# This script uses the stable "probability fusion" method.
# CRITICAL CHANGE: AMP is disabled (--use-amp is removed) to prevent 'inf' loss.
...
python main.py \
  ...
  --reset-epoch \
  --exper-name final_stable_fusion_no_amp \
  --gpu 0 \
  --epochs 15 \
  --batch-size 8 \
  --optimizer AdamW \
